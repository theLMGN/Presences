...
try {
    var PREMID_DEBUG_LOGGING = false
    ...
    console.log("[PreMiD] Initializing")
    var presence = new Presence({
        clientId: "676560908578717702",
        mediaKeys: false 
    })
    console.log("[PreMiD] Ready!")
    ...
    presence.on("UpdateData", () => {
        try {
            ...
            var username = loggedIn ? document.querySelector(".dropdown--account span").innerText : "Racing as a guest"
            var presenceData = {
                largeImageKey: "nt",
            ...
            try {
                if (path == "/") {
                    ...
                    var pName = document.querySelector(".has-btn--vertical .btn.is-active").innerText
                    presenceData.state = pName + " (" + (pName == "Summary" ? document.querySelector(".prog-points").innerText : document.querySelector(".twb").innerText).replace(/ /g,"") + ")"
                    ...
                    presenceData.state = document.querySelector(".profile-username").innerText
                    ...
                    var pos = parseInt(document.querySelector(".dash-pos .tsxxl").innerText)
                    var wpm = document.querySelector(".list--xs > li:nth-child(1) > div:nth-child(1) ").innerText.split("\n").reverse().join("").toLowerCase() 
                    var acc = document.querySelector(".list--xs > li:nth-child(2) > div:nth-child(1) > div:nth-child(2)").innerText + "acc"
                    ...
                    if (document.querySelector(".race-results")) {
                        presenceData.state = "Finished in " + document.querySelector("div.raceResults-title").innerText + " " + document.querySelector(".gridTable-row.is-self > div:nth-child(4) > div:nth-child(2) > div:nth-child(2)").innerText.replace(/ /g,"").replace(/\n/g," ")
                    }
                } else {
                    if (PREMID_DEBUG_LOGGING) {
    ...
    console.error("[PreMiD] Failed to load",e)
}